...
#slideshow img {
    ...
    display: block; /* Ensure all images are in block mode */
}
...
function showSlide(index) {
    ...
    currentSlide = index; // Update the index for the new slide
    if (currentSlide < 0) currentSlide = slides.length - 1; // Wrap around to last slide
    if (currentSlide >= slides.length) currentSlide = 0; // Wrap around to first slide
